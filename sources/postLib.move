...
//                 officialReply: object::id_from_address(@0x0),   // new transfer get 0 ID
//                 bestReply: object::id_from_address(@0x0),
//                 deletedReplyCount: 0,
//                 isDeleted: false,
//                 tags: postTags,
...
//     public entry fun createReply<postMetaData: copy>(
...
//         assert!(parentReplyId == object::id_from_address(@0x0) || (postMetaData.postType != EXPERT_POST && postMetaData.postType != COMMON_POST), E_USER_IS_FORBIDDEN_TO_REPLY_ON_REPLY_FOR_EXPERT_AND_COMMON_TYPE_OF_POST);

//         // TODO: add check role
//         assert!(!commonLib::isEmptyIpfs(ipfsHash), commonLib::getErrorInvalidIpfsHash());

//         // new transfer only 1 reply?
//         /*
...
//         if (parentReplyId == object::id_from_address(@0x0)) {
//             if (isOfficialReply) {
//                 postMetaData.officialReply = commonLib::getItemId(&mut replyUID); /*vector::length(&mut post.replies);*/
//             };

//             if (postMetaData.postType != TUTORIAL && postMetaData.author != userAddr) {
//                 if (getActiveReplyCount(postMetaData) == 0) {    // unit test
//                     isFirstReply = true;
//                     // TODO: add
...
//             if (newCommunityId != postMetaData.communityId && newCommunityId != DEFAULT_COMMUNITY /*&& !self.peeranhaUser.isProtocolAdmin(userAddr)*/) { // todo
...
//             postMetaData.officialReply = object::id_from_address(@0x0);
...
//                 };
//             };
//             if (postMetaData.bestReply != object::id_from_address(@0x0)) {
//                 // TODO: add
...
//             postMetaData.bestReply = object::id_from_address(@0x0);
...
//                 replyMetaData.parentReplyId == object::id_from_address(@0x0) && postMetaData.bestReply == commonLib::getItemId(&replyMetaData.id),
...
//         _communityId: u64,
//         replyMetaData: &mut ReplyMetaData,
//         _userCommunityRating: &mut userLib::UserCommunityRating
//     ) {
//         // TODO: add
//         // if (CommonLib.isEmptyIpfs(replyContainer.info.ipfsDoc.hash) || replyContainer.info.isDeleted)
//             // return;
//         // let isBestReply = replyMetaData.parentReplyId == object::id_from_address(@0x0);
...
//     // public entry fun voteForumItem(                          // del transfer?
...
//         let postType = postMetaData.postType;
//         let votedUserAddress = tx_context::sender(ctx);
//         assert!(votedUserAddress != replyMetaData.author, E_ERROR_VOTE_REPLY);

//         let (ratingChange, isCancel) = getForumItemRatingChange(votedUserAddress, &mut replyMetaData.historyVotes, isUpvote, &mut replyMetaData.votedUsers);
...
//     fun voteComment(
//         commentMetaData: &mut CommentMetaData,
//         _votedUser: &mut userLib::User,
//         _votedUserCommunityRating: &mut userLib::UserCommunityRating,
...
//         let _votedUserRating = i64Lib::zero();   // usersRating[1]
...
//                 _votedUserRating = i64Lib::mul(&getUserRatingChange(postType, RESOURCE_ACTION_DOWNVOTE, typeContent), &i64Lib::neg_from(1)); 
//             };

//             if (i64Lib::compare(&ratingChanged, &i64Lib::zero()) == i64Lib::getLessThan()) {
//                 _authorRating = i64Lib::mul(&_authorRating, &i64Lib::neg_from(1));
//                 _votedUserRating = i64Lib::mul(&_votedUserRating, &i64Lib::neg_from(1));
//             };
//         } else {
//             _authorRating = getUserRatingChange(postType, RESOURCE_ACTION_DOWNVOTED, typeContent);
//             _votedUserRating = getUserRatingChange(postType, RESOURCE_ACTION_DOWNVOTE, typeContent);
...
//                 _authorRating = i64Lib::mul(&_authorRating, &i64Lib::neg_from(1));
//                 _votedUserRating = i64Lib::mul(&_votedUserRating, &i64Lib::neg_from(2));  
//             };
//         };

//         // let _usersRating: vector<UserRatingChange> = vector<UserRatingChange>[UserRatingChange{ user: votedUser, userCommunityRating: votedUserCommunityRating, rating: _authorRating }, UserRatingChange{ user: votedUser, userCommunityRating: votedUserCommunityRating, rating: votedUserRating }];
...
//         communityLib::onlyExistingAndNotFrozenCommunity(community);
//         let _oldCommunityId: u64 = postMetaData.communityId;
...
//         let reply = vector::borrow_mut(&mut postMetaData.replies, replyId - 1);
//         reply
//     }
